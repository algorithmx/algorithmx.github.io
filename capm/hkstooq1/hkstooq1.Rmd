...
```{r}
h_95pct_mean_p <- which(mean_price  > quantile(mean_price,  probs=c(0.95), na.rm=TRUE))
h_95pct_mean_v <- which(mean_volume > quantile(mean_volume, probs=c(0.95), na.rm=TRUE))
h_95pct_max_p  <- which(max_price   > quantile(max_price,   probs=c(0.95), na.rm=TRUE))
