...
legend(newdate[36], 25000, c("[上海发布公众号]数据","Logistic模型,拟合+预测"), col=c("black","red"), 
       pch=c(1,1), lty=c(0,1), y.intersp=1.5)
...
We use the $\beta$-$\rho$ parametrisation of the SIR model to perform cost function minimization. The integrator for the new parametrisation of the SIR model is: 
...
fit_SIR_result <- nls(
          A ~ f(t, beta, rho, R0), 
          data=df,
          start = list(beta=10.0, rho=0.9, R0=0.5*A0),
...
SIR 模型的预测与人口增长 Logistic 模型的相近：清零预计大约在3月17日后的两个月。
