...
plot(  dates, population*diff(SIR_pred$R[msk]), 
       col="darkgreen", type="l", lwd=2, xlim=c(0,60), ylim=c(0,32000),
...
fit_SIR_result_n <- vector(mode="list",length=16L)
for (days in 15L:0L){
  df_n <- df[1:(nrow(df)-days),]
  fit_SIR_result_n[[16-days]] <- nls(
      A ~ f(t, beta, rho, R0),
      data=df_n,
      start = list(beta=10.0, rho=0.8, R0=(0.001+(16-days)*0.006)*A0),
      lower = c(0.1,  0.01, 0.0),
      upper = c(30.0, 1.0,   A0),
      algorithm = "port",
...
plot(df$date[(nrow(df)-15):nrow(df)], beta_fit, 
     col="blue", xlab="日期", ylab="beta")
lines(df$date[(nrow(df)-15):nrow(df)], beta_fit, col="blue")
axis.Date(1, at=seq(df$date[(nrow(df)-14)],df$date[nrow(df)],by=2))
title("SIR模型拟合beta值")
```
...
plot(df$date[(nrow(df)-15):nrow(df)], 1/rho_fit, 
     col="red", xlab="日期", ylab="R0")
lines(df$date[(nrow(df)-15):nrow(df)], 1/rho_fit, col="red")
axis.Date(1, at=seq(df$date[(nrow(df)-14)],df$date[nrow(df)],by=2))
title("SIR模型拟合推算R0值")
```
