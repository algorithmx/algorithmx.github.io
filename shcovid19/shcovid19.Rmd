...
Tn <- T1n + T2n
plot(df$date,T2n, col="blue",  ylim=c(0,6e5),
     xlab="日期", ylab="累计感染人数")
...
for (days in (LL-1L):0L){
  ...
  fit_SIR_result_n[[LL-days]] <- nls(
      A ~ f(t, beta, rho, R0),
      data=df_n,
      start = list(beta=10.0, rho=0.8, R0=(0.001+(LL-days)*0.006)*A0),
      lower = c(0.1,  0.01, 0.0),
      upper = c(30.0, 1.0,   A0),
      algorithm = "port",
...
axis.Date(1, at=seq(df$date[(nrow(df)-(LL-1L))],df$date[nrow(df)],by=2))
title("SIR模型拟合beta值")
```
